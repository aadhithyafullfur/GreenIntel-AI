...
const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-50 h-[72px] bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-xl border-b border-black/[0.08] dark:border-white/10 transition-colors duration-300">
      <div className="max-w-7xl mx-auto h-full px-4 sm:px-6 lg:px-8 flex items-center justify-between">

        {/* Left Side: Brand Logo Group */}
        <Link to={isAuthenticated ? "/projects" : "/"} className="flex items-center gap-3 group">
          <div className="relative flex items-center justify-center w-9 h-9 rounded-xl bg-gradient-to-tr from-orange-500 to-rose-500 shadow-md shadow-primary/20 transition-transform group-hover:scale-[1.03]">
            <Brain className="w-5 h-5 text-white" />
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-extrabold tracking-tight text-text-main flex items-center gap-2">
            ...
            <span className="text-[8.5px] text-text-muted font-semibold tracking-wider uppercase">
        ...
        <nav className="hidden md:flex items-center gap-1.5 bg-black/[0.04] dark:bg-white/[0.05] p-1 rounded-xl border border-black/[0.04] dark:border-white/10">
          {navLinks.map((link) => {
            ...
            return (
              <Link
                key={link.path}
                to={link.path}
                className={`relative px-4 py-1.5 text-xs font-bold rounded-lg transition-colors duration-200 ${isActive ? 'text-text-main' : 'text-text-muted hover:text-text-main'
                  }`}
              >
                {isActive && (
                  <motion.span
                    layoutId="active-nav"
                    className="absolute inset-0 bg-white dark:bg-white/10 rounded-lg shadow-sm border border-black/[0.06] dark:border-white/10 z-0"
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="md:hidden absolute top-[72px] left-0 right-0 bg-white/95 dark:bg-[#09090b]/95 backdrop-blur-2xl border-b border-black/[0.06] dark:border-white/10 px-4 py-6 space-y-4 shadow-2xl z-40 overflow-visible"
          >
